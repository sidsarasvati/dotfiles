...
** Modern TypeScript/React (Tree-sitter)

Tree-sitter provides modern syntax highlighting and structural editing.
Built into Emacs 29+, but requires language grammars to be compiled.

/Added: Dec 22, 2025/ - Part of "Real VibeCoding" project (learning Emacs properly).

*** How it works

1. =treesit-language-source-alist= - WHERE to download grammar source code (Git repos)
2. =treesit-install-language-grammar= - Compiles grammar ONCE (creates .dylib in =~/.emacs.d/tree-sitter/=)
3. Auto-install block ensures new machines get grammars on first launch (~30sec)

*** Manual commands (for understanding)

- Check if grammar exists: =M-: (treesit-language-available-p 'tsx)=
- Install one grammar: =M-x treesit-install-language-grammar RET tsx=
- See installed: =ls ~/.emacs.d/tree-sitter/=

*** Grammar Sources

#+begin_src emacs-lisp
  ;; Tree-sitter grammars for TypeScript/React
  ;; These Git repos contain C code that compiles to .dylib files
  ...
  ;; Auto-install missing grammars on startup (first launch compiles all)
  (dolist (grammar '(tsx typescript javascript json css))
    (unless (treesit-language-available-p grammar)
      (treesit-install-language-grammar grammar)))


  ;; Auto-activate tree-sitter modes for ALL web files
